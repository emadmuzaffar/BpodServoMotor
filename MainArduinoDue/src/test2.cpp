...
constexpr uint8_t kFrequencyOutputPin = 2;
constexpr Tc* kTimer = TC0;
constexpr uint32_t kTimerChannel = 0;
constexpr uint32_t kTimerPeripheralId = ID_TC0;
constexpr uint32_t kTimerClockHz = VARIANT_MCK / 2; // Arduino Due master clock = 84 MHz, so timer clock = 42 MHz.
constexpr uint32_t kFrequencyMultiplier = 100000;
constexpr uint32_t kMinFrequency = 500000;
...
class Motor {
    ...
    */
    ...
    static void startFrequencyOutput(uint32_t frequencyHz) {
        if (frequencyHz == 0) {
            return;
        }
        // RC must be at least 2 timer ticks.
        constexpr uint32_t maximumFrequencyHz = kTimerClockHz / 2;
        if (frequencyHz > maximumFrequencyHz) {
            frequencyHz = maximumFrequencyHz;
        }
        uint32_t rc = kTimerClockHz / frequencyHz;
        if (rc < 2) {
            rc = 2;
        }
        const uint32_t ra = rc / 2;
        // Enable the Timer Counter peripheral clock.
        pmc_set_writeprotect(false);
        pmc_enable_periph_clk(kTimerPeripheralId);
        // Connect Arduino pin 2 to its TIOA0 timer peripheral.
        PIO_Configure(
        ...
        TC_SetRA(kTimer, kTimerChannel, ra);
        TC_SetRC(kTimer, kTimerChannel, rc);
        TC_Start(kTimer, kTimerChannel);
    }

    void stopFrequencyOutput() {
        TC_Stop(kTimer, kTimerChannel);
        // Return pin 2 to normal GPIO control and hold it low.
        pinMode(kFrequencyOutputPin, OUTPUT);
        digitalWrite(kFrequencyOutputPin, LOW);
    }
    ...
    */
    void writePwmForRPM(const double rpm) const {
        ...
        const auto duty = static_cast<uint32_t>(std::lround(clampedRPM / static_cast<uint32_t>(MaxMotorRPM) * static_cast<uint32_t>(kFrequencyMultiplier))) + static_cast<uint32_t>(kMinFrequency);
        startFrequencyOutput(duty);
    ...
    */
    void pwmStop() {
        startFrequencyOutput(kMinFrequency);
        running = false;
        pidTarget = 0;
        lastError = 0;
        pidTime = 0;
